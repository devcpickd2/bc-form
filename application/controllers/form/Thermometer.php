...
class Thermometer extends CI_Controller {
	...
	public function cetak()
	{
		...
		$thermometer_data = $this->thermometer_model->get_by_date($tanggal, $plant); 
		$thermometer_data_verif = $this->thermometer_model->get_last_verif_by_date($tanggal, $plant); 

		if (!$thermometer_data || !$thermometer_data_verif) {
		...
		$data['thermometer']->nama_lengkap_produksi = $data['thermometer']->nama_produksi;
		...
		// 	}
			...
			if ($data['thermometer']->status_produksi == 1 && !empty($data['thermometer']->nama_produksi)) {
				...
				$pdf->SetFont('times', 'U', 8);
				$pdf->SetXY(90, $y_verifikasi + 10);
				$pdf->Cell(135, 5, $data['thermometer']->nama_produksi, 0, 1, 'C');
