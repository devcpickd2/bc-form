<div class="container-fluid">
    <h1 class="h3 mb-2 text-gray-800">Tambah Laporan Roti Gosong</h1>
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            <li class="breadcrumb-item">
                <a href="<?= base_url('gosong')?>">
                    <i class="fas fa-arrow-left"></i> Daftar Laporan Roti Gosong
                </a>
            </li>
            <li class="breadcrumb-item active" aria-current="page">Tambah</li>
        </ol>
    </nav> 
    <div class="card shadow mb-4">
        <div class="card-body">
            <form class="user" method="post" action="<?= base_url('gosong/tambah');?>">
                <?php
                $produksi_data = $this->session->userdata('produksi_data');
                $tanggal_sess = $produksi_data['tanggal'] ?? date('Y-m-d');
                $shift_sess = $produksi_data['shift'] ?? '';
                ?>
                <div class="form-group row">
                    <div class="col-md-6">
                        <label class="font-weight-bold">Tanggal</label>
                        <input type="date" name="date" class="form-control <?= form_error('date') ? 'is-invalid' : '' ?>"
                        value="<?= set_value('date', $tanggal_sess) ?>">
                        <div class="invalid-feedback"><?= form_error('date') ?></div>
                    </div>
                    <div class="col-md-6">
                        <label class="font-weight-bold">Shift</label>
                        <select id="shift_main" name="shift_main" class="form-control <?= form_error('shift_main') ? 'is-invalid' : '' ?>">
                            <option disabled <?= empty($shift_sess) ? 'selected' : '' ?>>Pilih Shift</option>
                            <option value="1" <?= set_select('shift_main', '1', $shift_sess == '1') ?>>Shift 1</option>
                            <option value="2" <?= set_select('shift_main', '2', $shift_sess == '2') ?>>Shift 2</option>
                            <option value="3" <?= set_select('shift_main', '3', $shift_sess == '3') ?>>Shift 3</option>
                        </select>
                        <div class="invalid-feedback"><?= form_error('shift_main') ?></div>
                    </div>
                </div>

                <div class="form-group row">
                    <div class="col-md-6">
                        <label class="form-label font-weight-bold">Grup Shift</label>
                        <select class="form-control <?= form_error('shift_group') ? 'is-invalid' : '' ?>" name="shift_group">
                            <option disabled selected>--Pilih Grup--</option>
                            <option value="A" <?= set_select('shift_group','A'); ?>>A</option>
                            <option value="B" <?= set_select('shift_group','B'); ?>>B</option>
                            <option value="C" <?= set_select('shift_group','C'); ?>>C</option>
                        </select>
                        <div class="invalid-feedback <?= !empty(form_error('shift_group')) ? 'd-block' : '' ; ?> ">
                            <?= form_error('shift_group') ?>
                        </div>
                    </div>
                    <div class="col-sm-6">
                        <label class="form-label font-weight-bold">Total Berat (Kg)</label>
                        <input type="text" name="total_berat" id="total_berat" class="form-control <?= form_error('total_berat') ? 'invalid' : '' ?>" value="<?= set_value('total_berat'); ?>" >
                        <div class="invalid-feedback <?= !empty(form_error('total_berat')) ? 'd-block' : '' ; ?>">
                            <?= form_error('total_berat') ?>
                        </div>
                    </div>

                </div>

                <div class="row">
                    <div class="col">
                        <button type="submit" class="btn btn-md btn-success mr-2">
                            <i class="fa fa-save"></i> Simpan
                        </button>
                        <a href="<?= base_url('gosong')?>" class="btn btn-md btn-danger">
                            <i class="fa fa-times"></i> Batal
                        </a>
                    </div>
                </div>
            </form>
        </div>
    </div>
</div>
</div>
<script>
    const totalBarang = document.getElementById('total_berat');

    totalBarang.addEventListener('blur', function() {
    let val = this.value.replace(',', '.'); // ganti koma jadi titik
    if (!isNaN(val) && val !== '') {
        // Paksa 3 digit di belakang koma
        let parts = parseFloat(val).toFixed(3).split('.');
        this.value = parts[0] + '.' + parts[1];
    } else {
        this.value = '';
    }
});
</script>
<style type="text/css">
    .breadcrumb{
        background-color: #2E86C1;
    }
</style>