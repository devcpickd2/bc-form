<div class="container-fluid">
    ...
    <nav aria-label="breadcrumb">
        ...
        <div class="card shadow mb-4">
            <div class="card-body">
                <form class="user" method="post" action="<?= base_url('analisis/statuslab/'.$analisis->uuid);?>">
                    <div class="form-group row">
                        <div class="col-sm-6">
                            <label class="form-label font-weight-bold">Status</label>
                            <select class="form-control <?= form_error('status_lab') ? 'invalid' : '' ?>" name="status_lab">
                                <option value="1" <?= set_select('status_lab', '1'); ?> <?= $analisis->status_lab == 1?'selected':'';?>>Accepted</option>
                                <option value="2" <?= set_select('status_lab', '2'); ?> <?= $analisis->status_lab == 2?'selected':'';?>>Returned</option>
                            </select>
                            <div class="invalid-feedback <?= !empty(form_error('status_lab')) ? 'd-block' : '' ; ?> ">
                                <?= form_error('status_lab') ?>
                            </div>
                        </div>
                    </div>
                    <div class="row form-group">
                        <div class="col-sm-6">
                            ...
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="catatan_lab" value="Sesuai" <?= ($analisis->catatan_lab == 'Sesuai') ? 'checked' : ''; ?>>
                                <label class="form-check-label">Sesuai</label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="catatan_lab" value="Tidak Sesuai" <?= ($analisis->catatan_lab == 'Tidak Sesuai') ? 'checked' : ''; ?>>
                                <label class="form-check-label">Tidak Sesuai</label>
                            </div>
                            <div class="invalid-feedback <?= !empty(form_error('catatan_lab')) ? 'd-block' : '' ; ?> ">
                                <?= form_error('catatan_lab') ?>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col">
                            ...
                            <a href="<?= base_url('analisis/diterima')?>" class="btn btn-md btn-danger">
