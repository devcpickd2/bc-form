    ...
    <nav aria-label="breadcrumb">
        ...
        <div class="card shadow mb-4">
            <div class="card-body">
                ...
                <hr>
                <?php
// $kondisi_map bisa dikosongkan untuk tambah
                $kondisi_map = isset($loading) ? json_decode($loading->kondisi_mobil, true) : [];
                ...
                <div id="form-rm" class="form-area mb-4">
                    <label class="form-label fw-bold mb-3">Kondisi Mobil</label>

                    <?php $chunks = array_chunk($kondisi_mobil_list, 5, true); ?>
                    <?php foreach ($chunks as $row): ?>
                        <div class="row mb-3">
                            <?php foreach ($row as $key => $label):
                                $selected_value = $kondisi_map[$key] ?? '';
                                ?>
                                <div class="col-lg-2 col-md-6 col-sm-12 mb-2">
                                    <div class="form-group">
                                        <label class="form-label"><?= $label ?></label>

                                        <input type="hidden" name="keterangan_kondisi[<?= $key ?>]" value="">

                                        <select class="form-control" name="keterangan_kondisi[<?= $key ?>]">
                                            <option value="" disabled <?= $selected_value === '' ? 'selected' : '' ?>>Pilih</option>
                                            <?php foreach ($keterangan_options as $opt): ?>
                                                <option value="<?= $opt ?>" <?= $selected_value === $opt ? 'selected' : '' ?>><?= $opt ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                </div>
