        ...
        <div class="card shadow mb-4">
            ...
            <div id="form-rm" class="form-area">
                ...
                <div class="container-fluid px-0">
                    <?php foreach (array_chunk($kondisi_mobil_list, 5) as $row): ?>
                        <div class="row mb-4">
                            <?php foreach ($row as $index => $kondisi): ?>
                                <div class="col-md-2">
                                    <label class="form-label font-weight-bold"><?= $kondisi ?></label>
                                    <?php foreach ($keterangan_options as $opt): ?>
                                        <?php 
                                        $id = 'kondisi-' . strtolower(str_replace(' ', '-', $kondisi)) . '-' . strtolower($opt);
                                        ?>
                                        <div class="form-check">
                                            <input class="form-check-input"
                                            type="checkbox"
                                            name="keterangan_kondisi[<?= $kondisi ?>][]"
                                            id="<?= $id ?>"
                                            value="<?= $opt ?>">
