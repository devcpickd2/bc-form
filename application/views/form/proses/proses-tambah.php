...
<script>
    document.addEventListener('DOMContentLoaded',function(){
        ...
        if (selectProduk) {
            const fillNamaProduk = () => {
                const val = selectProduk.value;

                namaInputs.forEach((input, i) => {
                    if (val === 'BC Mix Natural' || val === 'BC Mix Sintetis') {
                // 7 pertama jadi Yellow, sisanya Orange
                        input.value = (i < 7) ? 'BC Yellow' : 'BC Orange';
                    } else {
                        input.value = val;
                    }
                });
            ...
            fillNamaProduk();
            selectProduk.addEventListener('change', fillNamaProduk);
        }
        ...
        if(firstInput){
            firstInput.addEventListener('input', function(){
                ...
                if(!match){
                    for(let i=1; i<kodeInputs.length; i++) kodeInputs[i].value = '';
                        return;
                }
